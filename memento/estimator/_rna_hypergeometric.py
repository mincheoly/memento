...
class RNAHypergeometric(EstimatorBase):
    ...
    def covariance(self, data: sparse.csr_matrix, size_factor: np.array, idx1: np.array, idx2: np.array):
        ...
        if len(overlap_idx) >0:
            prod[overlap_location] = prod[overlap_location] - (1-self.q)*data[:, overlap_idx].T.multiply(row_weight**2).T.tocsr().sum(axis=0).A1/n_obs
    ...
    def bootstrap_mean(
        ...
        inv_sf_sq: np.array):
        """
            Computes the mean from bootstrapped expression values and frequencies. 
        ...
        return (values*freq*inv_sf).sum(axis=0)/n_obs
    ...
    def bootstrap_variance(
        ...
        mm_M1 = (values*freq*inv_sf).sum(axis=0)/n_obs
        mm_M2 = (values**2*freq*inv_sf_sq - (1-self.q)*data[0]*data[1]*inv_sf_sq).sum(axis=0)/n_obs
